...
io.on('connection', function (socket) { // socket pour avoir une instance différente pour chaque seveur
    ...
    ** 
    */
    socket.on('clientRequest', function (requestType, request[0], request[1], callback){ 
                    ...
                    */
                    ...
                    serveurRecuperationDonnees.emit('searchStopsNearToRealTime',
                        ...
                        if(err){
                            ...
                            // horaires obtenu en temps réel ou partiellement en temps réel
                            if(callback) callback(erreur, stopsNearToRealTime);
                        }
                        else{
                            console.log('Pas de temps réel, callbak des arrêts reçus par le SGBD');
                            // horaires prévu par la compagnie
                            if(callback) callback(erreur, stopsNearTo);
                        }
                    });
                }
                else {
